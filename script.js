...
async function loadInitialData() {
    try {
        ...
        const excelRes = await fetch('composer.xlsx'); if (!excelRes.ok) throw new Error('composer.xlsx не найден');
        const workbook = XLSX.read(new Uint8Array(await excelRes.arrayBuffer()), { type: 'array' });
        const rawData = XLSX.utils.sheet_to_json(workbook.Sheets[workbook.SheetNames[0]], { header: 1, range: 2 });
